...
export const useVideoFrame = (videoRef: RefObject<HTMLVideoElement | null>) => {
  ...
  const updateNextVideoCallback = useCallback(
    (callback: VideoFrameCallback | null) => {
      ...
      const runCallback: VideoFrameCallback = (now, metadata) => {
        ...
        nextFrameCallback?.(now, metadata);
      };
      ...
      if (
        ...
        requestHandleRef.current = {
          ...
          timeoutId: window.setTimeout(
            ...
            VIDEO_FRAME_CALLBACK_TIMEOUT_MS,
          ),
          token,
          video,
        };
